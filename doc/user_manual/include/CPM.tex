...
\begin{itemize}
  ...
  \item priorities: priority values assigne to each acitivty (optional)
\end{itemize}

The resources availability time series is provided in form of a pandas dataframe index by the 
...
where the parameter ``sgs'' indicates the strategy to select activities out of available candidates at each time step
(indicated as schedule generation scheme - SGS).
Allowed values for the ``sgs'' paramteres are:
\begin{itemize}
