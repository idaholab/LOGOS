...
\begin{figure}
    \centering
    \centerline{\includegraphics[scale=0.8]{image1.pdf}}
...
\begin{figure}
    \centering
    \centerline{\includegraphics[scale=0.8]{image2.pdf}}
...
We can compare the two options in two ways. The first way is to simply compute the {\it net}  NPV as the difference between the NPV of Option 1 and that of Option 2, i.e.,  \( \mbox{Net NPV}=\mbox{NPV Option }1-\mbox{NPV Option }2 \) . The resulting equation is:\par
...
\begin{figure}
    \centering
    \centerline{\includegraphics[scale=0.8]{image3.pdf}}
...
We can compute  \( \text{Net NPV} \)  as:\par
...
In order to use this external model to compute net NPV, the \xmlString{Logos.IncrementalNPV}
should be always used for \xmlAttr{subType}. Except the common nodes \xmlNode{variables}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    delaying specifies in \xmlNode{Td}. In order to communicate with RAVEN, these
...
The parameters \textbf{Cp, Cu, fp, Cd, inflation, tax} can be sampled by RAVEN. In
this case, if the user specifies these parameters in the input XML \xmlNode{ExternalModel},
